...
import type { LinkRecord, ValidatedLink, ScanStep, ScanResult } from '../types/scan.types';
...
function cleanError(err: unknown): string {
	...
	// strip ANSI chars from Playwright errors
...
async function safeGoto(page: Page, url: string): Promise<{ strategy: string; warning?: string }> {
	// Try strict first
	try {
...
async function collectResponsive(browser: any, url: string) {
	...
	const output: Array<{ viewport: string; hasHorizontalScroll: boolean; screenshot: string }> = [];
...
export async function runPlaywrightScan(urls: string[], scanId: string): Promise<ScanResult[]> {
		...
		for (const url of urls) {
			...
			try {
				...
				try {
					...
					const uniqueLinks = Array.from(new Map(normalizedLinks.map((l) => [l.href, l])).values());
					...
					result.interactive = await page.evaluate(() => {
						...
						const forms = Array.from(document.querySelectorAll('form')).map((f) => {
							...
							const inputs = Array.from(form.querySelectorAll('input, select, textarea')).map((i) => {
								const field = i as HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement;
								return {
				...
				// considered success if navigation step succeeded
