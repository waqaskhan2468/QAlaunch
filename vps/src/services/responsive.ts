...
// Viewport width threshold: ≤ this value → use slice capture instead of
...
async function scanViewport(
	...
	try {
		...
		if (isMobile) {
			// ── Mobile: capture overlapping slices ──────────────────────────
			// Each slice is ≤ 844px tall so Claude's vision API never downscales
			// them. See screenshots.ts → takeMobileSlices for full rationale.
			const { slices, sliceCount } = await takeMobileSlices(page);
			...
			return {
				...
				height: viewport.height,
				hasHorizontalScroll,
				screenshot: slices[0], // first slice — keeps existing DB field valid
				slices,
				sliceCount,
			};
		}
